...
#TASK 1
# Initialize variables for verbose and interface
interface=""
verbose="no"

# using loop to give the result of -v
while [[ $# -gt 0 ]]; do
    case "$1" in
        -v)
            verbose="yes"
...
# Get list of interface names dynamically
interfaces=$(ip -o link show | awk -F': ' '{print $2}' | grep -v "lo")

#TASK 2 Generate report for each interface except loopback
for int in $interfaces; do
    if [[ -z $interface || $interface == $int ]]; then
        if [[ $verbose == "yes" ]]; then
            echo "Report for interface $int:"
            ip -o addr show $int
        else
            echo "Interface $int:"
            ip -o addr show $int | awk '/inet / {print $4}'
        fi
    fi
done
